...
for index in range(len(TOI_list)):
    TOI_Name = 'TOI ' + str(TOI_list[index])
    obs_table = Observations.query_criteria(s_ra=RA[index], s_dec=DEC[index], project="TESS", dataproduct_type = "TIMESERIES", radius="0.002 deg", calib_level = 3, target_name= str(target_list['TIC'][index]))
